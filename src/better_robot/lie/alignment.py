...
def umeyama(
    ...
    *,
    ...
    if source.ndim < 2 or target.ndim < 2 or source.shape[-1] != 3 or target.shape[-1] != 3:
        raise ValueError(
            "source and target must have shape (..., N, 3); got "
            f"{tuple(source.shape)} and {tuple(target.shape)}"
        )
    if source.shape[-2] != target.shape[-2]:
        raise ValueError(
            f"source and target need the same N; got {source.shape[-2]} "
            f"and {target.shape[-2]}"
        )
    if source.shape[-2] < 3:
    ...
    if weights is None:
        ...
        fit_weights = weights.to(dtype=source.dtype, device=source.device)
        try:
            fit_weights = torch.broadcast_to(fit_weights, weight_shape)
        except RuntimeError as exc:
            raise ValueError(
                f"weights must broadcast to {tuple(weight_shape)}; got {tuple(fit_weights.shape)}"
            ) from exc

    if bool((fit_weights < 0).any()):
        raise ValueError("weights must be non-negative")
    weight_sum = fit_weights.sum(dim=-1, keepdim=True)
    if bool((weight_sum <= 0).any()):
        raise ValueError("weights must have a positive sum")
    normalized_weights = fit_weights / weight_sum
    ...
    source_variance = (
        normalized_weights * source_centered.square().sum(dim=-1)
    ).sum(dim=-1)
