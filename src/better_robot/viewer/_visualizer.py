...
class Visualizer:
    ...
    Typical usage (fixed-base)::
        ...
        while True:
            if vis.restart_requested:
                ...
                vis.reset_targets(robot, cfg)
            cfg = solve_ik(robot, targets=vis.get_targets(), ...)
            vis.update(cfg)

    Typical usage (floating-base)::

        vis = Visualizer(urdf, robot, floating_base=True)
        vis.add_grid()
        ...
        while True:
            if vis.restart_requested:
                ...
                vis.reset_targets(robot, cfg, base_pose)
            base_pose, cfg = solve_ik(robot, targets=vis.get_targets(), ...)
    ...
    def add_target(self, link_name: str, scale: float = 0.12) -> None:
        ...
        Call :meth:`reset_targets` after adding all targets to position them
        at the robot's current FK poses.
    ...
    def add_restart_button(self) -> None:
        ...
        :attr:`restart_requested` consumes it (returns ``True`` once, then
        resets to ``False``).
        """
    ...
    def reset_targets(
        ...
        Args:
            robot: Robot instance (used to compute FK).
        ...
        """
        fk = robot.forward_kinematics(cfg, base_pose=base_pose) if base_pose is not None \
            else robot.forward_kinematics(cfg)
    ...
    def get_targets(self) -> dict[str, torch.Tensor]:
        ...
        Returns:
            Dict mapping link name → ``(7,)`` SE3 tensor
    ...
    def update(
        ...
        self._urdf_handle.update_cfg(build_cfg_dict(self._robot, cfg))
