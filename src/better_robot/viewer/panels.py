...
def build_joint_panel(
    ...
    *,
    ...
    state = {"q": model.q_neutral.clone().clamp(model.lower_pos_limit, model.upper_pos_limit)}

    for j in range(model.njoints):
        jm = model.joint_models[j]
        nv_joint = model.nvs[j]
        if nv_joint == 0:
        ...
        for dof in range(nv_joint):
            ...
            label = f"{model.joint_names[j]}" if jm.nv == 1 else f"{model.joint_names[j]}[{dof}]"
