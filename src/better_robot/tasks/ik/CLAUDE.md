...
```python
from better_robot.tasks.ik import solve_ik, IKConfig, IKVariable
# or:
...
`initial_base_pose` is set → `_solve_floating()` which routes to `_solve_floating_autodiff()` or `_solve_floating_analytic()`.

**Returns**: Fixed base: `(num_actuated_joints,)` tensor. Floating base: `(base_pose (7,), cfg (n,))`.

No PyPose imports in `solver.py` — the floating-base path no longer uses `_FloatingBaseIKModule` or `ppo.LevenbergMarquardt`.
...
**`solver` / `solver_params`**: applies to the fixed-base path. Floating-base path uses its own internal LM loop.

## `IKVariable` (`variable.py`)

```python
@dataclass
class IKVariable:
    cfg: Tensor          # (n,) joint configuration
    base_pose: Tensor | None = None  # (7,) SE3 pose, or None for fixed base
```

Bundles the optimization variables for both fixed and floating-base IK.

- `is_floating() → bool`: True if base_pose is not None
- `to_flat() → Tensor`: Fixed-base returns `cfg (n,)`; floating-base returns `cat([cfg, zeros(6)])` — base tangent starts at zero
- `from_flat(flat, base_pose) → IKVariable`: Reconstruct from flat vector. For floating-base, applies SE3 retraction: `new_base = se3_exp(flat[n:]) @ base_pose`, then normalizes quaternion.

The flat representation `[cfg (n,) | base_tangent (6,)]` is the optimization variable passed to the LM solver in the floating-base path.
...
## Floating-Base Path (`_solve_floating`)

Routes based on `IKConfig.jacobian`:
- `jacobian="autodiff"` → `_solve_floating_autodiff()`: internal LM loop using `torch.func.jacrev` on the flat `[cfg, base_tangent]` variable
- `jacobian="analytic"` → `_solve_floating_analytic()`: custom LM loop with block-structured `(n+6, n+6)` system using `compute_jacobian` for joints and the adjoint-based formula for base
...
**Base update**: SE3 retraction in both modes: `base_new = se3_exp(delta_base) @ base`, then quaternion normalization.
