"""Robust grouped objective/gradient semantics for object residuals."""

from __future__ import annotations

import pytest
import torch

from better_robot.optim import Problem, Residual, Variable
from better_robot.optim.kernels import Cauchy, Huber, L2, Tukey


class _GroupedResidual(Residual):
    def __init__(self, x: Variable, target: Variable, *, weight=1.0, kernel=None) -> None:
        self.x, self.target = x, target
        super().__init__(x, target, dim=4, weight=weight, kernel=kernel, group_size=2, name="grouped")

    def error(self) -> torch.Tensor:
        return self.x.tensor - self.target.tensor


class _NonzeroAtOriginKernel:
    def __init__(self) -> None:
        self.rho_calls = 0

    def rho(self, squared_norm: torch.Tensor) -> torch.Tensor:
        self.rho_calls += 1
        return squared_norm + 7.0

    def weight(self, squared_norm: torch.Tensor) -> torch.Tensor:
        return torch.ones_like(squared_norm)


@pytest.mark.parametrize(
    "kernel",
    [L2(), Huber(delta=0.7), Cauchy(c=0.9), Tukey(c=2.5)],
    ids=["l2", "huber", "cauchy", "tukey"],
)
def test_public_objective_and_gradient_use_grouped_kernel_convention(kernel: object) -> None:
    target_tensor = torch.tensor([[-0.4, 0.2, 0.8, -0.3], [0.7, -0.6, 0.1, 0.9]], dtype=torch.float64)
    x_tensor = torch.tensor(
        [[0.3, -0.5, 1.2, 0.4], [-0.1, 0.5, -0.8, 0.2]],
        dtype=torch.float64,
        requires_grad=True,
    )
    x = Variable(x_tensor, name="x", batch_ndim=1)
    target = Variable(target_tensor, name="target", trainable=False, batch_ndim=1)
    multiplier = 1.7
    problem = Problem([_GroupedResidual(x, target, weight=multiplier, kernel=kernel)])

    weighted = multiplier * (x_tensor - target_tensor)
    groups = weighted.reshape(2, 2, 2)
    squared_norm = groups.square().sum(dim=-1)
    expected_cost = kernel.rho(squared_norm).sum(dim=-1)
    group_weight = kernel.weight(squared_norm).repeat_interleave(2, dim=-1)
    expected_gradient = multiplier * group_weight * weighted

    objective = problem.objective()
    gradient = problem.gradient()["x"]
    coordinate_gradient = torch.autograd.grad(objective.sum(), x_tensor)[0]
    torch.testing.assert_close(objective, expected_cost, rtol=1e-12, atol=1e-12)
    torch.testing.assert_close(gradient, expected_gradient, rtol=1e-11, atol=1e-11)
    torch.testing.assert_close(coordinate_gradient, expected_gradient, rtol=1e-11, atol=1e-11)


def test_inactive_residual_skips_kernel_with_nonzero_origin() -> None:
    kernel = _NonzeroAtOriginKernel()
    x = Variable(torch.ones(4), name="x")
    target = Variable(torch.zeros(4), name="target", trainable=False)
    item = _GroupedResidual(x, target, weight=0.0, kernel=kernel)
    problem = Problem([item])
    torch.testing.assert_close(problem.objective(), torch.tensor(0.0))
    torch.testing.assert_close(problem.gradient()["x"], torch.zeros(4))
    assert kernel.rho_calls == 0
