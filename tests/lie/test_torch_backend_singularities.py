...
def _singular_input(name: str, theta: float) -> torch.Tensor:
    ...
    if name == "so3_exp":
        return omega
    if name == "so3_log":
        return tn.so3_exp(omega).detach()

    xi = torch.cat([torch.zeros(3, dtype=torch.float64), omega])
    if name == "se3_exp":
        return xi
    if name == "se3_log":
        return tn.se3_exp(xi).detach()
...
def _fp32_cutoff_input(name: str, theta: float) -> torch.Tensor:
    ...
    xi = torch.tensor(
        [0.1, -0.2, 0.3, theta, 0.0, 0.0], dtype=torch.float32
    )
    if name == "se3_exp":
        return xi
    if name == "se3_log":
...
def test_fp32_taylor_cutoff_is_larger_than_fp64() -> None:
    assert tn._taylor_theta2(torch.float32) == 1e-5
    assert tn._taylor_theta2(torch.float64) == 1e-8
...
@pytest.mark.parametrize(
    "cutoff_ratio", [0.5, 2.0], ids=["taylor_branch", "full_branch"]
)
def test_fp32_gradcheck_on_both_sides_of_taylor_cutoff(
    name, op, cutoff_ratio
) -> None:
    ...
    """
    cutoff = tn._taylor_theta2(torch.float32)
    theta = math.sqrt(cutoff) * cutoff_ratio
    value = _fp32_cutoff_input(name, theta).requires_grad_(True)

    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore", message="Input #0 requires gradient and is not a double precision"
        )
