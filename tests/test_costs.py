...
def test_pose_residual_shape(panda):
    ...
    res = pose_residual(q, panda, hand_idx, target)
    assert res.shape == (6,)
...
def test_pose_residual_gradient(panda):
    q = panda._q_default.clone().requires_grad_(True)
    hand_idx = panda.link_index("panda_hand")
...
def test_manipulability_residual_shape(panda):
    ...
    r = manipulability_residual(q, panda, link_idx)
    assert r.shape == (1,)
