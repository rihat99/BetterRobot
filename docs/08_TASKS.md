...
@dataclass
class Trajectory:
    """First-class temporal type. Shape: (B, T, nq), (B, T, nv), (B, T)."""
    t:   torch.Tensor                # (B, T) timestamps
    q:   torch.Tensor                # (B, T, nq)
    v:   torch.Tensor | None = None  # (B, T, nv)
    a:   torch.Tensor | None = None
    u:   torch.Tensor | None = None  # (B, T, nu) control (optional)
    model_id: int = -1
...
cuRobo's default motion generator parameterises trajectories as a
cubic B-spline with `n_control_points ≪ T` control knots. Three wins:
...
- `src/better_robot/tasks/ik/solver.py` (585 lines of fixed/floating
  autodiff/analytic code) → replaced by `tasks/ik.py` (≤ 120 lines of
  facade).
- `src/better_robot/tasks/ik/variable.py` (`IKVariable` flat packing) →
  deleted. `q` *is* the flat variable; free-flyer and joint DOFs coexist in
  it.
- `src/better_robot/tasks/ik/config.py` (`IKConfig`) → split into
  `IKCostConfig` and `OptimizerConfig`.
- `src/better_robot/tasks/trajopt/` stub → replaced by `tasks/trajopt.py`
  skeleton with correct signatures.
- `src/better_robot/tasks/retarget/` stub → replaced by `tasks/retarget.py`
  skeleton.
