...
[13_NAMING.md](../conventions/13_NAMING.md). Implementing the bodies is a later phase —
...
come from [14_PERFORMANCE.md](../conventions/14_PERFORMANCE.md).
...
2. **Milestone D1 — `center_of_mass`.** Pure kinematic function, easy win.
   Inertia accumulation over topo order. **Close-out runs in parallel with
   D2** — RNEA does not depend on a finished centroidal map, only on stable
   Lie/spatial conventions and the `JointModel` dynamics hooks (§5.1).
...
6. **Milestone D5 — centroidal** (`compute_centroidal_map`, `ccrba`). Sits
   in parallel with D2/D3/D4 — pure kinematics.
7. **Milestone D6 — derivatives.** Carpentier & Mansard analytic derivative
   ...
   `jacrev` (see [14_PERFORMANCE.md §2.3](../conventions/14_PERFORMANCE.md)).
8. **Milestone D7 — three-layer action model** (§6 below).

### 5.1 `JointModel` dynamics hooks (precondition for D2)

The current `JointModel` Protocol covers kinematic operations
(`joint_transform`, `joint_motion_subspace`, `joint_velocity`, `integrate`,
`difference`). RNEA additionally needs the **spatial bias acceleration**
`c_J` and the **derivative of the motion subspace** `Ṡ_J(q,v)`. For
revolute, prismatic, and free-flyer these are identically zero; for
spherical, anatomical, or coupled joints they are non-zero.

Add the hooks to the Protocol now, with default implementations of zero in
the abstract base:
...
class JointModel(Protocol):
    ...
    def joint_bias_acceleration(
        self,
        q_slice: torch.Tensor,         # (B..., nq_j)
        v_slice: torch.Tensor,         # (B..., nv_j)
...
The simple kinds inherit the defaults; coupled and anatomical joints
([Proposal 09](../conventions/15_EXTENSION.md) seam) override them. Land
this with D0 / D1 — *before* D2 RNEA — so the dynamics surface does not
have to refactor the joint Protocol later.

The acceptance criterion for D2 reads as:

> RNEA computes `tau` via `S_J(q)` and `c_J(q,v)`; for any joint kind whose
> `joint_bias_acceleration(...)` returns non-zero, the RNEA forward pass
> uses it. Tested against a synthetic coupled-joint fixture and against a
> Pinocchio reference for the standard kinds.
...
[17_CONTRACTS.md §Autograd](../conventions/17_CONTRACTS.md).
...
[16_TESTING.md §4.5](../conventions/16_TESTING.md)).
