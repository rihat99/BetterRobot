...
@dataclass
class IRModel:
    schema_version: int = 1                 # bump on breaking IR-shape change
    name: str
    bodies: list[IRBody]
    joints: list[IRJoint]
    ...
    meta: dict = field(default_factory=dict)   # transit slot for parser hints — e.g. asset_resolver
```
...
`IRModel.schema_version` defaults to the current version. `build_model`
validates:
...
def build_model(ir: IRModel, *, free_flyer: bool = False) -> Model:
    if ir.schema_version != IRModel.schema_version:
        raise IRSchemaVersionError(
            f"IR has schema_version={ir.schema_version}; this build of "
            f"better_robot expects {IRModel.schema_version}. "
            f"Re-parse the source asset."
        )
    ...
```

Single-int counter; bumped in the same PR that changes the IR shape, with
a one-line entry in `docs/CHANGELOG.md`. Cached IR dumps (`.npz`
artifacts in `tests/io/`) carry the version; old fixtures regenerate when
the version bumps. `IRSchemaVersionError` lives in
`better_robot.exceptions` and is documented in
...
class ModelBuilder:
    ...
    (custom joint kinds registered via the
    `JointModel <../conventions/15_EXTENSION.md>`_ extension seam).
    *Do not* pass strings such as ``"revolute_z"`` to ``add_joint`` —
    the named helpers are the documented API.
    ...
    def add_prismatic(self, name, parent, child, axis, **kw) -> str: ...
    ...
    def add_free_flyer_root(self, name, child, **kw) -> str:
        """Convenience: free-flyer joint between universe and ``child``."""
    def add_helical(self, name, parent, child, axis, pitch, **kw) -> str: ...

    # ---- Catch-all for custom joint kinds (JointModel instances only) ----
    def add_joint(self, *, name: str, parent: str, child: str,
                  kind: "JointModel",
                  origin: "SE3 | torch.Tensor | None" = None,
                  lower=None, upper=None,
                  mimic_source=None, mimic_multiplier=1.0, mimic_offset=0.0) -> str: ...

    def add_frame(self, name: str, *, parent_body: str,
                  placement: "SE3 | torch.Tensor",
                  frame_type: str = "op") -> str: ...
    def add_collision_geom(self, body: str, kind: str, params: dict,
                           origin: "SE3 | torch.Tensor") -> None: ...
    def finalize(self) -> IRModel: ...
```

`add_joint(kind=...)` accepts a `JointModel` *instance*, not a string —
strings stop being valid for the `add_joint` path. A string-keyed call
(e.g. `add_joint(kind="revolute_z", ...)`) raises a typed error pointing
at `add_revolute_z`. This eliminates the historical `kind="revolute_z"`
vs `IRJoint(kind="revolute", axis=(0,0,1))` mismatch — the named helpers
*are* the friendly forms, so the bug cannot exist.
...
## 8. Already retired (reference)

The pre-skeleton parser layer disappeared as follows:

- `models/parsers/urdf.py` and `_urdf_impl.py` → `io/parsers/urdf.py`.
- `RobotURDFParser` → gone. The URDF parser is a module-level function
  returning an `IRModel` that `build_model()` consumes.
- `from_urdf` as a `classmethod` on `RobotModel` → gone. There is one
  construction entry point, `build_model(ir)`, and one user-facing
  loader, `load()`.
- `yourdfpy` imports anywhere outside `io/parsers/urdf.py` →
  forbidden by the layer-DAG contract test.
...
- Geometry/mesh resolution is delegated to an **`AssetResolver` Protocol**
  (see §11). The IR carries the URI string; the resolver maps it to a
  local path. Each parser builds a default resolver rooted at the source
  file's directory; users can override per-call via
  `parse_urdf(source, resolver=...)`. `Model.meta["asset_resolver"]` carries
  the resolver used at parse time so downstream consumers (viewer,
  collision SDF generation) inherit it.

## 11. Asset resolution — the `AssetResolver` Protocol

URDF and MJCF mesh paths come in three flavours:

- absolute filesystem paths;
- relative paths against the URDF's directory;
- ROS-style `package://<pkg>/<path>` URLs that need a package map.

Hand-rolling that logic in every parser, viewer, and collision-mesh loader
is the historical pain point. The Protocol:
...
| `CachedDownloadResolver` | for `robot_descriptions`-style packages that ship asset URLs; downloads to `~/.cache/better_robot/assets/` |
...
If `resolver=None`, the parser builds a `FilesystemResolver` rooted at the
URDF's directory — preserving today's behaviour. The same kwarg is on
`parse_mjcf`, on the `Visualizer` (so it doesn't reimplement path logic),
and on any future `RobotCollision` mesh loader. See
[12_VIEWER.md](12_VIEWER.md) for the integration.
