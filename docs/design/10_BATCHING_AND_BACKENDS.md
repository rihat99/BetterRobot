...
> [14_PERFORMANCE.md](../conventions/14_PERFORMANCE.md); this doc explains the
...
3. Never calling backend functions that break the autograd graph on
   CUDA. Today: `_pypose_backend` is the live default; the
   `residual_jacobian` path uses central finite differences to dodge a
   PyPose backward bug (see CLAUDE.md). After P10/L-A:
   `_torch_native_backend` becomes available and is gradcheck-clean by
   construction; PyPose stays reachable under `BR_LIE_BACKEND=pypose`
   for regression testing through L-C, then is removed (see
   [03_LIE_AND_SPATIAL.md §10](03_LIE_AND_SPATIAL.md)).
...
## 7. Backends — Protocol-driven, explicit objects

The architectural core is **explicit `Backend` objects**. A
process-global default exists as convenience sugar
(`default_backend()`, `set_backend("warp")`), but every algorithm can
take an explicit `backend: Backend | None` override via a `backend=`
kwarg. Reasons the global is sugar, not the architectural core:

- `torch.compile` traces against captured globals; switching the global
  mid-trace silently invalidates compiled artefacts.
- Tests that compare two backends side by side need both live in the
  same process without monkey-patching.
...
def default_backend() -> "Backend":
    """The current process default (one-time configuration)."""

current = default_backend          # convenience alias
...
def set_backend(name: BackendName) -> None:
    """One-time configuration. Avoid calling from inside library code;
    pass an explicit backend= kwarg instead."""
```

### 7.1 The `Backend` Protocol
...
|---------|-------|--------------|------------|--------|
| `pypose` (current default) | full | full (via lie) | stub | shipped; default until P10/L-C; removed in v1.2 (L-D) |
| `torch_native` | full, pure-PyTorch | full | RNEA/CRBA/ABA stub until D2-D4 | planned (P10/L-A); becomes default at L-C |
| `warp` | partial | partial — FK kernel | stub | experimental |

### 7.3 Where the backend boundary lives
...
| `dynamics/rnea.py` / `aba.py` / `crba.py` | yes (when implemented) |
| `spatial/*.py` | indirectly (call into `lie/`) |
| `data_model/joint_models/*.py` | indirectly |
| Everything else | no |

A new contract test (`tests/contract/test_backend_boundary.py`) AST-walks
`src/` and fails if any module outside `lie/`, `kinematics/`, `dynamics/`,
or `backends/<name>/` imports a backend implementation module.

### 7.4 Warp backend roadmap
...
(see [14_PERFORMANCE.md §5](../conventions/14_PERFORMANCE.md)).
...
1. **Explicit per-call (the primary mechanism):**
...
2. **Process-wide default (convenience sugar):**

   ```python
   br.backends.set_backend("warp")  # one-time configuration
   ```

   Library-internal code never calls `set_backend` (a contract test greps
   for it). User code calls it once at startup.

3. `kinematics/forward.py` reads `backend or default_backend()` and
   dispatches. The torch-native backend implements FK as described in 05;
   the Warp backend dispatches to `backends/warp/kernels/fk.py`.
4. `lie/se3.py` routes through the same `backend or default_backend()` 
   pattern, so the PyPose → Warp swap is also a backend switch.
