...
This document specifies the pinocchio-style **universal** data model that
replaces the current `RobotModel` / `RobotData` / `JointInfo` / `LinkInfo`
quartet. The static vs floating-base split disappears: a floating base is
simply a `JointFreeFlyer` root joint. A ball joint is simply a
`JointSpherical`.

> **Naming.** Field names below use the readable convention from
> [13_NAMING.md](13_NAMING.md). Pinocchio equivalents (`oMi`, `oMf`,
> `liMi`, `nle`, `Ag`, `hg`, …) are carried as deprecated aliases during
> the migration window (see §11 below).
...
class Data:
    ...
    # 0 = nothing computed, 1 = placements, 2 = velocities, 3 = accelerations
...
- **`_kinematics_level`** mirrors Pinocchio's "kinematic level" invariant — it
  lets functions that need `v`-level kinematics assert that level-2 has been
  computed.
...
- **Readable identifiers.** Every field is named by what it is. The pinocchio
  ...
  [13_NAMING.md](13_NAMING.md) for the full rename table and rationale.
...
- `src/better_robot/models/robot_model.py` → replaced by `data_model/model.py`
- `src/better_robot/models/data.py` → replaced by `data_model/data.py`
- `src/better_robot/models/joint_info.py` → absorbed into `data_model/model.py`
- `src/better_robot/models/link_info.py` → replaced by `data_model/body.py` + `frame.py`
- `RobotModel._frozen` custom guard → replaced by `@dataclass(frozen=True)`
- `RobotModel._fk_joint_parent_link / _fk_joint_child_link / _fk_joint_types /
  ...
  replaced by proper `parents`, `topo_order`, `joint_models`, `idx_qs`, etc.
- The notion of "base_pose" as a separate argument — folded into `q[0:7]` via
  `JointFreeFlyer`. The entire `_solve_floating_*` family in the current IK
  solver vanishes.

## 11. Deprecated-name shims (migration window)

During the rename sprint (see
[11_SKELETON_AND_MIGRATION.md §Rename sprint](11_SKELETON_AND_MIGRATION.md)),
`Data` additionally exposes the old names as `@property` shims backed by the
new fields. Accessing any of them emits a `DeprecationWarning`:

```python
@property
def oMi(self):
    warnings.warn("Data.oMi → Data.joint_pose_world; see docs/13_NAMING.md",
                  DeprecationWarning, stacklevel=2)
    return self.joint_pose_world
```

The shim set is exactly the left column of [13_NAMING.md §4](13_NAMING.md).
