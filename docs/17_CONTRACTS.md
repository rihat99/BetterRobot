...
[10_BATCHING_AND_BACKENDS.md](10_BATCHING_AND_BACKENDS.md) for the
full shape table.
...
| `BackendNotAvailableError` | `backends.set_backend` | Requested backend (e.g. `warp`) not importable | Install extra (`uv add 'better-robot[warp]'`) |
...
- The 25-symbol public API (`better_robot.__all__`).
- The SE(3) quaternion layout.
...
| `tasks/` | Stable from v1 for IK; `solve_trajopt` and `retarget` are **experimental** until phase `T`. |
...
| `io/` (URDF/MJCF) | Stable. Parser edge cases may iterate in patch releases. |
