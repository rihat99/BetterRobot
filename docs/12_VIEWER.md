...
(still accessible as `model.meta["ir"]`). Each geom is tessellated
through `trimesh` and pushed to the backend via `add_mesh`. Analytical
primitives (`<box>`, `<cylinder>`, `<sphere>`, `<capsule>`) are handled
by `trimesh.creation.*`. Loading failures skip the geom silently.
...
body raises `NotImplementedError("see docs/12_VIEWER.md §10.*")`.
