...
LM is the default. Switch to `"gn"` for Gauss–Newton, `"adam"` for
first-order, or `"lbfgs"` for quasi-Newton.

## Robust kernels

Pose, position, and orientation residuals can be reweighted with a
robust kernel:
...
Available kernels: `l2`, `huber`, `cauchy`, `tukey`.
