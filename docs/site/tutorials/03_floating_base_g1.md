...
position) but `nv=6` (twist). `solve_ik` handles this transparently;
no extra `initial_base_pose` argument is needed.
...
The retraction inside the LM step is SE(3)-aware on the base joint and
linear on revolutes — this is what makes "one code path" possible.

See `concepts/lie_and_spatial.md` for the Lie-algebra background.
