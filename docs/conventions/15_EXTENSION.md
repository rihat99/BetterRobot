...
class JointCoupled(JointModel):
    ...
    def joint_bias_acceleration(self, q, v) -> torch.Tensor:        # (B..., 6)
        """c_J — spatial bias acceleration. Required by RNEA D2."""
    def joint_motion_subspace_derivative(self, q, v) -> torch.Tensor:  # (B..., 6, nv_j)
        """Ṡ_J — used by RNEA forward pass and by analytic dynamics derivatives."""
```
...
[02_DATA_MODEL.md §Joint Model Protocol](../design/02_DATA_MODEL.md)):
...
[07_RESIDUALS_COSTS_SOLVERS.md](../design/07_RESIDUALS_COSTS_SOLVERS.md) for the
full schema).
...
[09_COLLISION_GEOMETRY.md](../design/09_COLLISION_GEOMETRY.md).
...
Contract (`RenderMode` Protocol — see [12_VIEWER.md](../design/12_VIEWER.md)).
...
See [04_PARSERS.md](../design/04_PARSERS.md).
...
[10_BATCHING_AND_BACKENDS.md §Warp bridge](../design/10_BATCHING_AND_BACKENDS.md).
...
**Use when:** you want a different optimisation variable shape for trajopt
(e.g. Lin/Cosine basis, segmented polynomials, time-warped knot grids).
The Protocol owns the mapping `z ↔ Trajectory`:
...
`BSplineTrajectory` (cuRobo-style smooth). See
[08_TASKS.md §3.1](../design/08_TASKS.md).
...
Concrete resolvers shipped in core: `FilesystemResolver`, `PackageResolver`,
`CompositeResolver`, `CachedDownloadResolver`. See
[04_PARSERS.md §11](../design/04_PARSERS.md).

## 14 · Backend extension

**Use when:** swapping the tensor primitive layer (Warp, JAX, custom CUDA
kernels). The contract is the `Backend` Protocol from
[10_BATCHING_AND_BACKENDS.md §7.1](../design/10_BATCHING_AND_BACKENDS.md):

```python
# my_package/backends/jax/__init__.py
class JaxBackend:
    name = "jax"
    lie:        LieOps         # see protocol.py
...
Register either by passing explicit `backend=BACKEND` kwargs at every
call site, or by editing `backends/__init__.py::_load` to recognise the
new name. The user-facing type stays `torch.Tensor`; conversion happens
inside the backend's autograd-Function wrappers.

## 15 · Adding actuators (e.g. muscles)

**Use when:** you have a non-rigid actuator that contributes to joint
torque (DeGrooteFregly muscles, splined dampers, etc.).
...
## 16 · What is **not** pluggable (deliberately)
...
| `IRModel` shape | The `schema_version` field is the controlled change vector — see [04 §2.1](../design/04_PARSERS.md). |
