...
> [13_NAMING.md](13_NAMING.md). If you are porting Pinocchio code, the old
...
```python
def _fk_impl(model: Model, q: torch.Tensor) -> tuple[Tensor, Tensor]:
    B = q.shape[:-1]
...
### Why this replaces the current FK
...
The existing `tasks/ik/solver.py` has:

- `_solve_fixed()` with a `_build_fixed_jacobian_fn` that stacks
  `compute_jacobian`, `limit_jacobian`, `rest_jacobian` by hand.
- `_solve_floating_analytic()` with an inline block-structured Jacobian
  using an adjoint trick for the base column.
- `_solve_floating_autodiff()` with a full `torch.func.jacrev` call over a
  flat `[q, base_tangent]` vector.
- `_analytic_collision_jacobian()` as a separate sparse-collision Jacobian.

Four Jacobian paths, all duplicating structure. The redesign folds these
into **one Jacobian strategy** that:
...
class JacobianStrategy(str, Enum):
    ANALYTIC   = "analytic"     # call residual.jacobian(state) if implemented
...
### `get_joint_jacobian(model, data, joint_id, reference="world") -> Tensor`
...
def get_joint_jacobian(
    ...
    joint_id: int,
    *,
    reference: Literal["world", "local", "local_world_aligned"] = "world",
) -> torch.Tensor:  # (B..., 6, nv)
...
automatically handled because `get_frame_jacobian` walks the topology and
joint 1 is just another joint in the chain. **This is what replaces the
current adjoint trick** in `_solve_floating_analytic()`.
