...
- Pose residual Jacobian: **analytic via `Jr_inv(log_err)`** (no autodiff
  through `Log` — PyPose's gradient is wrong there; see
  [03_LIE_AND_SPATIAL.md §Numerical stability](03_LIE_AND_SPATIAL.md)).
...
- The LM solver pre-allocates `JᵀJ` and `Jᵀr` buffers sized from the
  residual spec (see [07_RESIDUALS_COSTS_SOLVERS.md](07_RESIDUALS_COSTS_SOLVERS.md)).
...
[09_COLLISION_GEOMETRY.md](09_COLLISION_GEOMETRY.md)); the LM solver
...
CI runs `pytest tests/bench/ --benchmark-compare --benchmark-fail=mean:20%`.
A 20% regression against the committed baseline (stored in
`tests/bench/baseline.json`) fails the build.
...
|--------|------|--------------------------------|
| `lie/` | SE3/SO3 group ops | Short closed-form; compile-friendly |
...
| `dynamics/*.py` | RNEA/ABA/CRBA | Analytic derivatives (phase D3); compile-friendly recursion |
| `residuals/*.py` | Pure functions | Analytic `.jacobian()` where possible |
| `costs/stack.py` | Concatenation | Flat buffer, write-into-slice |
| `optim/optimizers/*.py` | Solver loops | Pre-allocated buffers; graph-capture-ready |
| `optim/solvers/*.py` | Linear solves | `torch.linalg.cholesky_ex`; fall back to `lstsq` |
| `collision/*.py` | SDF pairs | Sparsity-aware residual; vectorised pairs |
| `io/*.py` | One-shot parse | Not hot; readability > speed |
| `viewer/*.py` | Scene updates | 60 fps budget (see [12_VIEWER.md](12_VIEWER.md)) |
